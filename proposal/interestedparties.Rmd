...
responses[, c('lon', 'lat') := geocode(location, messaging = FALSE)]
...
We already have more than `r round(sum(promises == 'Presenting at a satRday') , -1)` presenters, roughly `r round(sum(promises == 'Attending a satRday') , -1)` attendees and `r round(sum(promises == 'Helping improve the website') , -1)` webmasters -- what can go wrong?